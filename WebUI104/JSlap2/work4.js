...
function removeButton(){
    var buttonEle = document.getElementById("pButton");
    var chk = $('#inChild').val();
    if(chk.length === 0 || chk <= 0){ 
        alert("Please enter the correct number.");
        return ;
    }
    if(chk > buttonEle.children.length){
        alert("The index is more then the children.");
        return ;
    }
    buttonEle.removeChild(buttonEle.children[chk]);
    sowElement(buttonEle);
}